   ...
   $contar = mysql_num_rows($sql3);
       ...
       exit;
            ...
            $ID_User = $row["ID_Usuario"];
             ...
             $sql4= mysql_query("SELECT * from Comentario_Foto where (ID_Usuario = '$ID_User') and (ID_Foto = '$ID_Foto') and (Departamento = '$Departament') and (Estado = 'Activo')");

   $contar4 = mysql_num_rows($sql4);
             
            if($contar4 == 0){
            echo "<script type=\"text/javascript\">
           alert('Usuario No Ha Hecho Ningun Comentario en La Foto');
           location.href = '$Ruta';
        //  history.go(-1);
       </script>";
       exit;   
...
$Query = "Update Comentario_Foto
Set Estado = 'Inactivo' Where (ID_Usuario = '$ID_User') and (ID_Foto = '$ID_Foto') and (Departamento = '$Departament')";
